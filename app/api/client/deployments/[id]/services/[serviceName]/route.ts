...
/**
 * Managed service removal (client scope).
 *
 * Tenancy is enforced inside the service layer: a deployment belonging to a
 * different client account resolves to NOT_FOUND (never FORBIDDEN), so a
 * direct-object-id probe cannot confirm existence. CLIENT_OPERATOR/VIEWER lack
 * `workload.edit` and are refused.
 */
