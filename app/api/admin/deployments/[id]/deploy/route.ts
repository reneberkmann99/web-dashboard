...
import { requestDeploymentOperation } from "@/server/services/deployment-executor";
import { deploySchema } from "@/server/validation/deployment";
import { cuidParamSchema } from "@/server/validation/admin";
import { fromError, fail, ok } from "@/server/http";
import { getSourceIpFromRequest } from "@/server/request";

/**
 * Queue a managed deployment. Executes through the curated agent API only,
 * gated on secure transport + a fresh (non-stale) plan.
 */
