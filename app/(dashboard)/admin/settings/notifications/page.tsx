...
export default function NotificationsSettingsPage(): React.JSX.Element {
  ...
  const noDeliveries = deliveries.length === 0;
  // A destination-less, event-less platform doesn't need two empty panels
  // saying the same thing (design review round 2, §5).
  const showDeliveryHistory = !(noDestinations && noDeliveries);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title="Alerting"
        description="Global webhook destinations, delivery policy and alert-pipeline health."
        // On a zero state the empty-state's own "Add webhook" is the right
        // action; a second filled button in the header just duplicates it.
        actions={!noDestinations ? <Button onClick={() => setCreateOpen(true)}><Plus className="mr-2 h-4 w-4" /> Add webhook</Button> : undefined}
      />
      ...
      <section>
        <h2 className="mb-3 border-b border-border pb-2 text-[15px] font-semibold">Destinations</h2>
        {destinationsQuery.isLoading ? <div className="h-32 animate-pulse rounded-lg bg-panelAlt" /> : noDestinations ? <div className="rounded-lg border border-border bg-panelAlt/40 p-5 text-sm"><p className="text-text">No destination configured. {noDeliveries ? "0" : deliveries.length} event{deliveries.length === 1 ? "" : "s"} queued.</p><p className="mt-1 text-muted">Noderaft still records every notification event{noDeliveries ? "" : " in the delivery history below"} — nothing is lost while delivery is unconfigured.</p><Button className="mt-3" size="sm" onClick={() => setCreateOpen(true)}><Plus className="mr-1 h-3 w-3" /> Add webhook</Button></div> : <div className="grid gap-3 lg:grid-cols-2">{destinations.map((destination) => <div key={destination.id} className="rounded-lg border border-border bg-panel p-4">
          <div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><Bell className="h-4 w-4 text-accent" /><p className="font-medium">{destination.name}</p><Badge variant={destination.enabled ? "success" : "neutral"}>{destination.enabled ? "enabled" : "disabled"}</Badge></div><p className="mt-1 text-xs text-muted">{destination.urlMasked}</p></div>{destination.lastDeliveryStatus && <Badge variant={statusVariant(destination.lastDeliveryStatus)}>{destination.lastDeliveryStatus.toLowerCase()}</Badge>}</div>
      ...
      {showDeliveryHistory && (
      <section>
        <h2 className="mb-3 border-b border-border pb-2 text-[15px] font-semibold">Delivery history</h2>
