...
export default function NotificationsSettingsPage(): React.JSX.Element {
  ...
  return (
    <div className="space-y-6">
      ...
      {failing.length > 0 && <div className="rounded-lg border border-danger/40 bg-danger/5 p-4"><div className="flex items-center gap-2 text-red-200"><TriangleAlert className="h-4 w-4" /><p className="font-medium">Notification destination failing</p></div>{failing.map((item) => <p key={item.id} className="mt-1 text-sm text-muted">{item.name}: {item.consecutiveFailures} consecutive failures. This warning is internal and does not recursively notify itself.</p>)}</div>}

      <section>
        <h2 className="mb-2 text-lg font-semibold">Destinations</h2>
        {destinationsQuery.isLoading ? <div className="h-32 animate-pulse rounded-lg bg-panelAlt" /> : destinations.length === 0 ? <div className="rounded-lg border border-border bg-panelAlt/40 p-5 text-sm text-muted">No destination configured. HostPanel still records logical notification events.</div> : <div className="grid gap-3 lg:grid-cols-2">{destinations.map((destination) => <div key={destination.id} className="rounded-lg border border-border bg-panel p-4">
          ...
          <div className="mt-4 flex flex-wrap gap-2"><Button size="sm" variant="secondary" onClick={() => sendTest.mutate(destination.id)} disabled={!destination.enabled || sendTest.isPending}><FlaskConical className="mr-1 h-3 w-3" /> Send test</Button><Button size="sm" variant="ghost" onClick={() => toggle.mutate({ id: destination.id, enabled: !destination.enabled })}>{destination.enabled ? "Disable" : "Enable"}</Button></div>
        </div>)}</div>}
      </section>
      ...
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="Add webhook destination" description="Credentials are encrypted and never shown again after creation." footer={<><Button variant="secondary" onClick={() => setCreateOpen(false)}>Cancel</Button><Button onClick={() => create.mutate()} disabled={create.isPending || !name || !url || signingSecret.length < 16 || eventTypes.length === 0}>Create</Button></>}>
        <div className="space-y-3"><label className="block text-sm">Display name<input className="mt-1 w-full rounded-md border border-border bg-panelAlt px-3 py-2" value={name} onChange={(event) => setName(event.target.value)} maxLength={100} placeholder="Operations Webhook" /></label><label className="block text-sm">URL<input type="url" className="mt-1 w-full rounded-md border border-border bg-panelAlt px-3 py-2" value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://hooks.example.com/…" /></label><label className="block text-sm">Authorization header value (optional)<input type="password" autoComplete="new-password" className="mt-1 w-full rounded-md border border-border bg-panelAlt px-3 py-2" value={authHeader} onChange={(event) => setAuthHeader(event.target.value)} placeholder="Bearer …" /></label><label className="block text-sm">HMAC signing secret<input type="password" autoComplete="new-password" className="mt-1 w-full rounded-md border border-border bg-panelAlt px-3 py-2" value={signingSecret} onChange={(event) => setSigningSecret(event.target.value)} minLength={16} placeholder="At least 16 characters" /></label><label className="block text-sm">Minimum severity<select className="mt-1 w-full rounded-md border border-border bg-panelAlt px-3 py-2" value={minSeverity} onChange={(event) => setMinSeverity(event.target.value as typeof minSeverity)}><option value="INFO">Info</option><option value="WARNING">Warning</option><option value="CRITICAL">Critical</option></select></label><fieldset><legend className="text-sm">Events</legend><div className="mt-2 grid gap-2 sm:grid-cols-2">{availableEvents.map(([value, label]) => <label key={value} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={eventTypes.includes(value)} onChange={(event) => setEventTypes(event.target.checked ? [...eventTypes, value] : eventTypes.filter((item) => item !== value))} className="accent-accent" />{label}</label>)}</div></fieldset><p className="text-xs text-muted">Private/RFC1918 webhook destinations are disabled by default. Operators may explicitly enable them with WEBHOOK_ALLOW_PRIVATE_NETWORKS when an internal receiver is intentional; link-local/cloud metadata remains blocked.</p></div>
      </Modal>
    </div>
  );
}
